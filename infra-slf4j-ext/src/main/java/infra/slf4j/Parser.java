...
public class Parser {
	...
	public String readQuotedString() throws IOException {
		if (start >= lenght) throw new EOFException();
		char c = chars[start];
		if (c != STRING_DELIM) throw new IOException("missing quotes");
		start++;
		int end = start;
	...
	public String readIdentifierString() throws IOException {
		...
		char c = chars[start];
		if (! Character.isJavaIdentifierStart(c))  throw new IOException("invalid identifier");
		int end = start + 1;
		while (end < lenght) {
			c = chars[end];
			if (! Character.isJavaIdentifierPart(c) && c != '.') break;
	...
	public void readOperator(char operator) throws IOException {
		while (start < lenght && Character.isWhitespace(chars[start])) start++;
		if (start >= lenght) throw new EOFException();
		char c = chars[start++];
		if (c != operator) throw new IOException("missing '"+operator+"'");
		while (start < lenght && Character.isWhitespace(chars[start])) start++;
	}

	public boolean readOptionalOperator(char operator) {
		while (start < lenght && Character.isWhitespace(chars[start])) start++;
		if (start >= lenght) return false;
		char c = chars[start];
	...
	public long readLong() throws IOException {
		...
		if (c=='-') {
			...
			if ((start+1) >= lenght) throw new EOFException();
			c = chars[start+1];
			end = start + 2;
		} else {
			end = start + 1;
	...
	public double readDouble() throws IOException {
		...
		if (! Character.isDigit(c) && c != '.') throw new NumberFormatException();
